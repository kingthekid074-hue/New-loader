...
local function ToggleNoDoors(state)
    ...
    if getgenv().NoDoors then
        task.spawn(function()
            while getgenv().NoDoors do
                local GameMap = workspace:FindFirstChild("MAPS") and workspace.MAPS:FindFirstChild("GAME MAP")
                
                if GameMap and GameMap:FindFirstChild("Doors") then
                    for _, obj in ipairs(GameMap.Doors:GetDescendants()) do
                ...
                for _, obj in ipairs(workspace:GetDescendants()) do
                    if obj:IsA("BasePart") then
                        ...
                        if string.find(nameLower, "door") or string.find(nameLower, "hitbox") then
                            if obj.CanCollide and obj.Transparency >= 0.5 then
                                obj.CanCollide = false
                            end
...
Tabs.Main:Section({ Title = "Auto Barricade" })
Tabs.Main:Toggle({
    Title = "Auto Barricade",
    Default = getgenv().AutoBarricade,
    Callback = function(state)
        getgenv().AutoBarricade = state
    end
})
